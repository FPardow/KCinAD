...
#reorder svm annotation
roj_rds@meta.data[["svm_annotation"]] <- factor(roj_rds@meta.data[["svm_annotation"]], levels = c("basal I", "basal II", "basal III", "basal IV", "stratum spinosum", "stratum granulosum")) #order cell identities

levels(roj_rds$svm_annotation) #check

#reorder disease factor 
...
#Stem cell markers
VlnPlot(roj_rds, group.by = "svm_annotation", split.by = "disease", features = c("COL17A1", "ITGB1", "ITGA6"), stack = T, flip = T, cols = colors_disease) + theme(axis.title.x = element_blank()) #i also tried "LRIG1", "DLL1", "MCSP", "CD46", "FRMD4A", "OCT4", "MYC", "OCT3", "SOX2", "KLF4"

VlnPlot(roj_rds[ , roj_rds$disease == "HC"], group.by = "svm_annotation", split.by = "disease", features = c("COL17A1", "ITGB1", "ITGA6"), stack = T, flip = T, cols = c(rep("#8DA0CB", 10))) + theme(axis.title.x = element_blank()) + NoLegend() #i also tried "LRIG1", "DLL1", "MCSP", "CD46", "FRMD4A", "OCT4", "MYC", "OCT3", "SOX2", "KLF4" 
...
FeaturePlot(roj_rds, features = c("JUN", "FOS", "TP63"), split.by = "disease") & theme(legend.position = "right")
FeaturePlot(roj_rds, features = c("CCL2", "POSTN", "IL1R2"), split.by = "disease") & theme(legend.position = "right")
FeaturePlot(roj_rds[, roj_rds$disease == "AD"], features = c("IGFBP3", "KRT6A", "KRT16", "S100A7", "S100A8", "KRT17", "CASP14", "SERPINB2", "DSG1")) & theme(legend.position = "right")

FeaturePlot(roj_rds, features = c("KRT15", "AHR", "CYP1A1", "CYP1B1"), split.by = "disease") & theme(legend.position = "right")

FeaturePlot(roj_rds[, roj_rds$disease == "HC"], features = c("AHR", "CYP1B1"), blend = T) & theme(legend.position = "right")

VlnPlot(roj_rds, group.by = "svm_annotation", features = c("AHR", "ARNT", "CYP1A1", "CYP1B1"), split.by = "disease", stack = T, flip = T, pt.size = 0, cols = colors_disease) + theme(axis.title.x = element_blank()) 

VlnPlot(roj_rds, group.by = "svm_annotation", split.by = "disease", features = c("AHR", "ARNT", "CYP1A1", "CYP1B1"), split.by = "disease", stack = T, flip = T, cols = colors_disease) + theme(axis.title.x = element_blank())

FeaturePlot(roj_rds[, roj_rds$disease == "HC"], features = c("AHR", "CYP1B1"), blend = T, cols = c("red", "blue", "green", "lightgrey")) & theme(legend.position = "right")

VlnPlot(roj_rds, group.by = "svm_annotation", split.by = "disease", features = c("AHR", "ID1", "TCF3", "BMP1", "BMP2", "BMP4"),  stack = T, flip = T, cols = colors_disease) + theme(axis.title.x = element_blank())

VlnPlot(roj_rds, group.by = "svm_annotation", split.by = "disease", features = c("TP63", "ADAM17"),  stack = T, flip = T, cols = colors_disease) + theme(axis.title.x = element_blank())

FeaturePlot(roj_rds, features = c("TP63", "ADAM17"), split.by = "disease") & theme(legend.position = "right")
```
...
VlnPlot(roj_rds[, roj_rds$disease == "HC"], group.by = "svm_annotation", features = c(m_bIII, m_bII, m_bI, m_bIV, "KRT5", "KRT14", "KRT1", "KRT10"), cols = c(rep("#8DA0CB", 10), rep("#66C2A5", 7), rep("#FFD92F", 10), c(rep("#A6D854", 7)), rep("#FC8D62", 4)), stack = T, flip = T) + NoLegend() + theme(axis.title.x = element_blank())

VlnPlot(roj_rds[, roj_rds$disease == "HC"], group.by = "svm_annotation", features = c(m_bI, m_bII, m_bIII, m_bIV, "KRT5", "KRT14", "KRT1", "KRT10"), cols = c(rep("#FFD92F", 10), rep("#66C2A5", 7), rep("#8DA0CB", 10), c(rep("#A6D854", 7)), rep("#FC8D62", 4)), stack = T, flip = T) + NoLegend() + theme(axis.title.x = element_blank())

VlnPlot(roj_rds, group.by = "svm_annotation", features = c(m_bI, m_bII, m_bIII, m_bIV, "KRT5", "KRT14", "KRT1", "KRT10"), cols = colors_disease, split.by = "disease", stack = T, flip = T) + theme(axis.title.x = element_blank())
...
DimPlot(roj_rds[ , roj_rds$disease == "HC"], group.by = "Phase", cols = colors_cellcycle, order = c("G2M", "S", "G1"))
...
#reorder: 
phase_percentage$Variable <- factor(phase_percentage$Variable, levels = c("basal I", "basal II", "basal III", "basal IV", "stratum spinosum", "stratum granulosum"))
...
##visualization
GO_HC_vis <- DEHC_basalIup_GO[c(1, 2)] 
GO_HC_vis <- rbind(GO_HC_vis, DEHC_basalIIup_GO[c(1, 2)])
GO_HC_vis <- rbind(GO_HC_vis, DEHC_basalIIIup_GO[c(4, 36)])
...
GO_HC_vis$celltype <- c(rep("basal I", 2), rep("basal II", 2), rep("basal III", 2), rep("basal IV", 2), rep("spinosum", 2), rep("granulosum", 3))
GO_HC_vis$celltype <- factor(GO_HC_vis$celltype, levels = c("basal I", "basal II", "basal III", "basal IV", "spinosum", "granulosum"))
...
org <- c("Human")
features <- 1000
diffOrder <- c("basal I", "basal II", "basal III", "basal IV", "stratum spinosum", "stratum granulosum")
pathway_order <- c("JAK-STAT", "WNT", "Estrogen", "MAPK", "VEGF", "EGFR", "PI3K", "Androgen", "TGFb", "TNFa", "NFkB", "Hypoxia", "Trail", "p53")
...
progeny_hmap <- pheatmap(t(summarized_progeny_scores_df_HC), 
                        ...
                        main =  paste0("PROGENy pathway activity HC"), angle_col = c("45")) #fontsize = 14, fontsize_row = 10
```
...
progeny_hmap <- pheatmap(t(summarized_progeny_scores_df_AD), 
                        ...
                        cluster_rows = F, #treeheight_row = 0, 
                        color = myColor, breaks = progenyBreaks, border_color = NA,
                        main =  paste0("PROGENy pathway activity AD"), angle_col = c("45")) #fontsize = 14, fontsize_row = 10
```
...
```{r progeny KRT15 vs ASS1}
objectrds <- roj_rds[, roj_rds$disease == "AD"]
# objectrds <- objectrds[, objectrds$svm_added_anno == "basal III KRT15+" | objectrds$svm_added_anno == "basal III ASS1+"]
Idents(objectrds) <- "svm_added_anno"
...
progeny_hmap <- pheatmap(t(summarized_progeny_scores_df),
                         ...
                         cluster_cols = F, treeheight_col = 0, cluster_rows = T, treeheight_row = 0, 
                         border_color = NA, 
                         main =  paste0("PROGENy (", features , " features)"), fontsize = 14, fontsize_row = 10, angle_col = c("45"))
progeny_hmap
```
...
colorder <- c("HC_basal I", "HC_basal II", "HC_basal III", "HC_basal IV", "HC_stratum spinosum", "HC_stratum granulosum", "AD_basal I", "AD_basal II", "AD_basal III", "AD_basal IV", "AD_stratum spinosum", "AD_stratum granulosum")
...
Idents(objectrds) <-  paste0(objectrds$disease,"_",objectrds$svm_annotation)
  ...
  progeny_hmap <- pheatmap(t(summarized_progeny_scores_df),
                           ...
                           cluster_rows = F, cluster_cols = F, angle_col = c("45"), treeheight_col = 0,  border_color = NA, 
                           color = myColor, breaks = progenyBreaks, 
...
FeaturePlot(roj_rds,features = c("slingPseudotime_1")) + ggtitle("pseudotime") + scale_color_gradient2(low = "yellow",mid = "yellow",high ="red",na.value = "grey") #x1
...
## Visualize all highly variable / differentially expressed genes along the pseudotime trajectory 
...
heatmap_hcad_cc <- rbind(c3, c5, c6) 
heatmap_hcad_early <- rbind(c4, c7) 
```

```{r GO check heatmap HC and AD}
c3_GO <- enrichGO(gene = rownames(c3), OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
c3_GOdf <- as.data.frame(c3_GO)
View(c3_GOdf)

c5_GO <- enrichGO(gene = rownames(c5), OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
c5_GOdf <- as.data.frame(c5_GO)
View(c5_GOdf)

barplot(c5_GO, showCategory = c5_GO$Description[c(1, 2)])

c6_GO <- enrichGO(gene = rownames(c6), OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
c6_GOdf <- as.data.frame(c6_GO)
View(c6_GOdf)

barplot(c6_GO, showCategory = c6_GO$Description[c(1, 2)])

heatmap_hcad_cc_GO <- enrichGO(gene = rownames(heatmap_hcad_cc), OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)

heatmap_hcad_cc_GOdf <- as.data.frame(heatmap_hcad_cc_GO)
View(heatmap_hcad_cc_GOdf)

barplot(heatmap_hcad_cc_GO, showCategory = heatmap_hcad_cc_GO$Description[c(186, 268)]) #proliferation
...
## [...] early in the pseudotime trajectory 
...
heatmap_hcad_early_downKRT15 <- c(rownames(c4), rownames(c5))
heatmap_hcad_early_downASS1 <- c(rownames(c1), rownames(c2))
heatmap_hcad_early_nochange <- c(rownames(c3))

downKRT15_GO <- enrichGO(gene = heatmap_hcad_early_downKRT15, OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
downKRT15_GOdf <- as.data.frame(downKRT15_GO)
View(downKRT15_GOdf)

barplot(downKRT15_GO, showCategory = downKRT15_GO$Description[c(4, 12)])

downASS1_GO <- enrichGO(gene = heatmap_hcad_early_downASS1, OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
downASS1_GOdf <- as.data.frame(downASS1_GO)
View(downASS1_GOdf)

barplot(downASS1_GO, showCategory = downASS1_GO$Description[c(1, 3, 17)])
...
barplot(Nochange_GO, showCategory = Nochange_GO$Description[c(1)])

#save
list <- list("Early_downKRT15" = downKRT15_GO,
             "Early_downASS1" = downASS1_GO,
             "Early_nochange" = Nochange_GO
             )
write.xlsx(list, file = "GO Heatmap HCADtrajectory early2.xlsx")

HCAD_early_GO <- downASS1_GO[c(1, 3, 17)]
HCAD_early_GO <- rbind(HCAD_early_GO, downKRT15_GO[c(4, 12)])
HCAD_early_GO <- rbind(HCAD_early_GO, Nochange_GO[c(1)])

HCAD_early_GO$celltype <- c(rep("cluster 1, 2", 3), rep("downregulated KRT15+", 2), rep("cluster 3", 1))
HCAD_early_GO$celltype <- factor(HCAD_early_GO$celltype, levels = c("cluster 1, 2", "cluster 4, 5", "cluster 3"))

ggplot(HCAD_early_GO, aes(x = Count, y = Description)) +
        geom_col(aes(fill = p.adjust)) +
        facet_grid("celltype", scale ="free") +
        scale_fill_gradient(high = "blue", low = "red", guide = guide_colourbar(reverse = TRUE)) +
        theme_bw(base_size = 13) + theme(axis.text.y = element_text(size = 13)) +
...
```{r check if those groups fit the KRT15 / ASS1}
# heatmap_hcad_early_downASS1 = c1, c2 -> KRT15
# heatmap_hcad_early_downKRT15 = C4, C5 -> ASS1

#KRT15 markers
# table(heatmap_hcad_early_downASS1 %in% markers[markers$AD_avg_log2FC > 0, "gene"])
table(heatmap_hcad_early_downASS1 %in% diffex_ad_basalIII[diffex_ad_basalIII$AD_avg_log2FC > 0, "gene"])
length(diffex_ad_basalIII[diffex_ad_basalIII$AD_avg_log2FC > 0, "gene"])
# table(heatmap_hcad_early_nochange %in% markers[markers$AD_avg_log2FC > 0, "gene"])
# table(c(heatmap_hcad_early_downASS1, heatmap_hcad_early_nochange) %in% markers[markers$AD_avg_log2FC > 0, "gene"])
KRT15_marker <- heatmap_hcad_early_downASS1[heatmap_hcad_early_downASS1 %in% markers[markers$AD_avg_log2FC > 0, "gene"]]
KRT15_marker <- append(KRT15_marker, heatmap_hcad_early_nochange[heatmap_hcad_early_nochange %in% markers[markers$AD_avg_log2FC > 0, "gene"]])

roj_rds_AD_KRT15 <- roj_rds[ , roj_rds$disease == "AD" & roj_rds$svm_added_anno == "basal III KRT15+"]

counts_sum <- rowSums(roj_rds_AD_KRT15@assays[["RNA"]])
background <- rownames(roj_rds_AD_KRT15@assays[["RNA"]][counts_sum > 1])

KRT15_marker_GO <- enrichGO(KRT15_marker, OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
KRT15_marker_GO <- as.data.frame(KRT15_marker_GO)
View(KRT15_marker_GO)

# TGFB: JUN/SPRY2/SPRY1/FOS/DKK3
#response to xenobiotic stimulus: TXNIP/JUN/FOS/ALDH3A1/ITGA2
# 
# candidate_KRT15 <- c("KRT15", "SMOC2", "DCN", "SPRY2", "SPRY1", "DKK3", "IL1R2", "LTF", "KREMEN2", "TXNIP", "EGR2",  "TMEM173", "SERTAD4",  "BIRC3", "ARHGAP29",  "BIRC3", "ADAMTS4", "CXCL2", "ALDH3A1", "ITGA2", "ARL4C")

FeaturePlot(roj_rds, features = c("KRT15",  "IL1R2", "SPRY1", "TXNIP"), split.by = "disease") 
#"TXNIP", "SMOC2" "SPRY1"
#"ALDH3A1"
#"DCN", "ITGA2"
#"TMEM173", "KREMEN2", 
...
#ASS1 markers
table(heatmap_hcad_early_downKRT15 %in% markers[markers$AD_avg_log2FC < 0, "gene"])
table(heatmap_hcad_early_downKRT15 %in% diffex_ad_basalIII[diffex_ad_basalIII$AD_avg_log2FC < 0, "gene"])

roj_rds_AD_ASS1 <- roj_rds[ , roj_rds$disease == "AD" & roj_rds$svm_added_anno == "basal III ASS1+"]

counts_sum <- rowSums(roj_rds_AD_ASS1@assays[["RNA"]])
background <- rownames(roj_rds_AD_ASS1@assays[["RNA"]][counts_sum > 1])

table(heatmap_hcad_early_nochange %in% markers[markers$AD_avg_log2FC < 0, "gene"])
ASS1_marker <- heatmap_hcad_early_downKRT15[heatmap_hcad_early_downKRT15 %in% markers[markers$AD_avg_log2FC < 0, "gene"]]
ASS1_marker <- append(ASS1_marker, heatmap_hcad_early_nochange[heatmap_hcad_early_nochange %in% markers[markers$AD_avg_log2FC < 0, "gene"]])



ASS1_marker <- enrichGO(ASS1_marker, OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
ASS1_marker <- as.data.frame(ASS1_marker)
View(ASS1_marker)

FeaturePlot(roj_rds, features = c("ASS1", "SPON2"), split.by = "disease") 
"ADIRF"
WNT10A, CAV1, WNT4,
S100A2 -> stress
S100A10, ITGA6, ITGB1 -> cell adhesion / plasma membrane 

FeaturePlot(roj_rds, features = c("KRT15", "SPON2", "ASS1", "IL1R2"), split.by = "disease") 
```

## [...] late in the pseudotime trajectory 
...
heatmap_hcad_late$gene <- rownames(heatmap_hcad_late)
View(heatmap_hcad_late)
```
...
```{r}
roj_rds[, roj_rds$disease == "AD"] <- roj_rds[ , roj_rds$disease == "AD" & roj_rds$svm_added_anno == "basal III KRT15+"]

#what to filter out
AD_KRT15 <- roj_rds[ , roj_rds$disease == "AD" & !is.na(roj_rds$svm_added_anno) & roj_rds$svm_added_anno == "basal III KRT15+"]
length(colnames(AD_KRT15))

AD_ASS1 <- roj_rds[ , roj_rds$disease == "AD" & !is.na(roj_rds$svm_added_anno) & roj_rds$svm_added_anno == "basal III ASS1+"]
length(colnames(AD_ASS1))

table(roj_rds_ad$svm_annotation == "basal III")

#filter out 
rojs_rds_AD_KRT15 <- roj_rds[ , !colnames(roj_rds) %in% colnames(AD_ASS1)]
rojs_rds_AD_ASS1 <- roj_rds[ , !colnames(roj_rds) %in% colnames(AD_KRT15)]
```

```{r changes basal III HC to KRT15+ cells  in AD}
change_krt15 <- FindMarkers(rojs_rds_AD_KRT15, group.by = "disease", subset.ident = "basal III", ident.1 = "AD", ident.2 = "HC") #comparison of KRT15+ changes in AD compared to HC, log2FC up means more expressed in first group (AD)
change_krt15$gene <- rownames(change_krt15)

change_krt15_GO <- enrichGO(change_krt15[change_krt15$avg_log2FC > 0, "gene"], OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
change_krt15_GOdf <- as.data.frame(change_krt15_GO)
View(change_krt15_GOdf)

change_krt15_GOdown <- enrichGO(change_krt15[change_krt15$avg_log2FC < 0, "gene"], OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
change_krt15_GOdowndf <- as.data.frame(change_krt15_GOdown)
View(change_krt15_GOdowndf)

```


```{r}
change_ass1 <- FindMarkers(rojs_rds_AD_ASS1, group.by = "disease", subset.ident = "basal III", ident.1 = "AD", ident.2 = "HC") #comparison of KRT15+ changes in AD compared to HC, log2FC up means more expressed in first group (AD)
change_ass1$gene <- rownames(change_ass1)

change_ass1_GOup <- enrichGO(change_ass1[change_ass1$avg_log2FC > 0, "gene"], OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
change_ass1_GOupdf <- as.data.frame(change_ass1_GOup)
View(change_ass1_GOupdf)

change_ass1_GOdown <- enrichGO(change_ass1[change_ass1$avg_log2FC < 0, "gene"], OrgDb = org.Hs.eg.db, universe = row.names(roj_rds@assays$RNA@counts), keyType = 'SYMBOL', ont = "ALL", readable = T)
change_ass1_GOdowndf <- as.data.frame(change_ass1_GOdown)
View(change_ass1_GOdowndf)


heatmap_hcad_early_downASS1[heatmap_hcad_early_downASS1 %in% change_ass1[change_ass1$avg_log2FC < 0, "gene"]]
heatmap_hcad_early_downASS1[heatmap_hcad_early_downASS1 %in% change_ass1[change_ass1$avg_log2FC > 0, "gene"]]

table(heatmap_hcad_early_downKRT15 %in% change_ass1[change_ass1$avg_log2FC > 0, "gene"])
```
...
## Differences basal III subtypes - AD

```{r DEanalysis AD KRT15 vs ASS1}
table(roj_rds$svm_added_anno)

markers <- FindMarkers(roj_rds[, roj_rds$disease == "AD"], group.by = "svm_added_anno", ident.1 = "basal III KRT15+", ident.2 = "basal III ASS1+")
colnames(markers) <- paste("AD", colnames(markers), sep = "_")
markers$gene <- rownames(markers)

diffex_ad_basalIII <- markers[markers$AD_p_val_adj < 0.05, ]
...
DEAD_KRT15down <-  enrichGO(gene = diffex_ad_basalIII[diffex_ad_basalIII$AD_avg_log2FC < 0, "gene"], 
                        universe = background, OrgDb = org.Hs.eg.db, keyType = 'SYMBOL', ont = "ALL",
                        readable = T)
DEAD_KRT15down_df <- as.data.frame(DEAD_KRT15down)
View(DEAD_KRT15down_df)

DEAD_ASS1down <-  enrichGO(gene = diffex_ad_basalIII[diffex_ad_basalIII$AD_avg_log2FC > 0, "gene"], 
                        universe = background, OrgDb = org.Hs.eg.db, keyType = 'SYMBOL', ont = "ALL",
                        readable = T)
DEAD_ASS1down_df <- as.data.frame(DEAD_ASS1down)
View(DEAD_ASS1down_df)
```
