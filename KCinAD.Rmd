...
# diffex_hc <- readRDS("diffex_hc.rds")
...
custom_colors <- c("basal III" = "#8DA0CB", "basal II" = "#66C2A5", "basal I" = "#FFD92F", "basal IV" = "#A6D854", "stratum spinosum" = "#FC8D62", "stratum granulosum" = "#E78AC3")

custom_colors_b3sub <- c("basal III KRT15+" = "#6573A0", "basal III ASS1+" = "#B0B9E6", "basal II" = "#66C2A5", "basal I" = "#FFD92F", "basal IV" = "#A6D854", "stratum spinosum" = "#FC8D62", "stratum granulosum" = "#E78AC3")
```
...
DimPlot(object, group.by = "svm_annotation",  cols = custom_colors,  repel = T,  label = T,  label.box = T) + NoLegend()
...
Idents(roj_rds) <- roj_rds$svm_annotation
DimPlot(roj_rds, cols = custom_colors,  repel = T,  label = T,  label.box = T) + NoLegend()
# DimPlot(roj_rds[, roj_rds$disease == "HC"], cols = custom_colors,  repel = T,  label = T,  label.box = T) + NoLegend()
# DimPlot(roj_rds[, roj_rds$disease == "AD"], cols = custom_colors,  repel = T,  label = T,  label.box = T) + NoLegend()
...
custom_colors_b3sub <- c("basal III KRT15+" = "#6573A0", "basal III ASS1+" = "#B0B9E6", "basal II" = "#66C2A5", "basal I" = "#FFD92F", "basal IV" = "#A6D854", "stratum spinosum" = "#FC8D62", "stratum granulosum" = "#E78AC3")
DimPlot(roj_rds[, roj_rds$disease == "AD"], cols = custom_colors_b3sub, repel = T,  label = T,  label.box = T) + NoLegend()
...
```{r stacked barplot cell type percentage}
...
custom_colors <- c("G1" = "darkorange", "S" = "#66C2A5", "G2M" = "#FFD92F")
DimPlot(roj_rds, group.by = "Phase", cols = custom_colors, order = c("G2M", "S", "G1"))
...
# plot(reducedDims(object_sce)$UMAP, col = custom_colors[as.factor(object_sce$svm_annotation)], pch = 16,  asp = 0.5)
...
#Plot pseudotime on UMAP
plot(reducedDims(object_hc_sce)$UMAP, col = custom_colors[as.factor(object_hc_sce$svm_annotation)], pch = 16,  asp = 0.5)
...
ggplot(dat, aes(x = svm_annotation, y = slingPseudotime_1, fill = svm_annotation, color = svm_annotation)) +
    ...
    scale_fill_manual(values = custom_colors) + scale_y_continuous(breaks = c(0, 5, 10, 15, 20, 25)) +
...
#Plot pseudotime on UMAP
plot(reducedDims(object_ad_sce)$UMAP, col = custom_colors[as.factor(object_ad_sce$svm_annotation)], pch = 16,  asp = 0.5)
...
# plot(reducedDims(roj_hc_sce)$UMAP, col = custom_colors[as.factor(roj_hc_sce$svm_annotation)], pch = 16,  asp = 0.5)
# lines(test, lwd = 4)
# 
# roj_ad_sce <- as.SingleCellExperiment(roj_rds_ad, assay = "RNA")
# plot(reducedDims(roj_ad_sce)$UMAP, col = custom_colors[as.factor(roj_ad_sce$svm_annotation)], pch = 16,  asp = 0.5)
...
#Plot pseudotime on UMAP
plot(reducedDims(roj_rds_sce)$UMAP, col = custom_colors[as.factor(roj_rds_sce$svm_annotation)], pch = 16,  asp = 1)
...
For doing this, a fitting model is needed to infer a smoothed gene expression along the pseudotime trajectory. We first use evaluateK to  helps us to select a good number of knots to fit on. 
...
# should i use logtransformed counts here or raw counts -> check input on fitGAM
```

Now we choose a subset of highly expressed genes that we will later crosscheck for highly variable expression. Only these genes are inferred along the pseudotime trajectory (for reasons of calculation time). 
...
Now we use a negative binomial general additive model to infer the smoothed gene expression of the highly expressed and highly variable genes with the knots we have determined before. 

```{r calculate NB-GAM}
# setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
# roj_rds_sce_gam <- readRDS("roj_rds_sce_gam.rds")
...
set.seed(123)
roj_rds_sce_gam <- fitGAM(counts = as.matrix(roj_rds_sce@assays@data$counts), sds = SlingshotDataSet(roj_rds_sce), nknots = 6, parallel = T)                      # genes = intersect(roj_rds@assays$RNA@var.features, rownames(gene_expr[gene_expr$color == "yes", ]))
                        #genes to fit, default = all
#used logtransformed counts before roj_rds@assays$RNA@data, now raw counts

#statistical test to check if gene expression changes across trajectory
diffex_trajectory <- associationTest(roj_rds_sce_gam, lineages = T) #lineages = T -> test for all lineages independently 
diffex_trajectory$gene <- rownames(diffex_trajectory)

# setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
# saveRDS(roj_rds_sce_gam, "roj_rds_sce_gam.rds")
# write.xlsx(diffex_trajectory, file = "diffex_trajectory.xlsx")

diffex_trajectory <- diffex_trajectory[diffex_trajectory$pvalue_1 < 0.05, ]
pseudotime_genes <- intersect(diffex_trajectory$gene, rownames(gene_expr[gene_expr$highly_expressed == "yes", ]))
```

HOW MANY KNOTS TO USE?
use only subset of genes for pseudotime gam modelling?

```{r}
length(roj_rds@assays$RNA@var.features)
length(rownames(gene_expr[gene_expr$highly_expressed == "yes", ]))

table(roj_rds@assays$RNA@var.features %in% rownames(gene_expr[gene_expr$highly_expressed == "yes", ]))
```


Now we draw a heatmap to visualize gene expression across the trajectory. We use only the highly and differential expressed genes for this that also are found highly variable. 

talk to ward about integrated vs non-integrated -> THEN optimize heatmap clustering 

```{r heatmap gene expression differentiation healthy}
library(ComplexHeatmap)

roj_rds_hc <- subset(roj_rds, disease == "HC")
genes_to_anno <- c("PTTG1", "KRT5", "KRT14", "UHRF1", "KRT6A", "ASS1", "KRT15", "KRT16", "KRT1", "KRT10", "FLG") #genes to annotate
custom_colors <- c("basal III" = "#8DA0CB", "basal II" = "#66C2A5", "basal I" = "#FFD92F", "basal IV" = "#A6D854", "stratum spinosum" = "#FC8D62", "stratum granulosum" = "#E78AC3")

#healthy trajectory
genes_heatmap <- c(genes_to_anno, genes_to_anno)
counts_hc <- as.data.frame(roj_rds_hc@assays$RNA@counts)
heatmap_df <- counts_hc[genes_heatmap, ] # filter for desired genes
colpt <- as.data.frame(roj_rds_hc@meta.data[ , c("slingPseudotime_1","svm_annotation")]) #obtain pseudotime
colpt$cells  <-  rownames(colpt)
colpt <- na.omit(colpt)
heatmap_df <- heatmap_df[ , colpt$cells] # remove cells not included in pseudotime
heatmap_df <- heatmap_df[ , order(colpt$slingPseudotime_1)] # order by pseudotime
annotationnames <- colpt[order(colpt$slingPseudotime_1), ]$svm_annotation
annotationnames <- annotationnames[!is.na(annotationnames)]

heatmap_df <- t(apply(heatmap_df, 1, function(x){scale(x)})) #change values into z score
heatmap_df <- t(apply(heatmap_df, 1, function(x){zoo::rollmean(x, 100, align = "left")})) # apply rolling average to rows
rownames(heatmap_df) <- genes_heatmap
heatmap_df <- na.omit(heatmap_df)

#annotate cell types
annot_celltype <- HeatmapAnnotation(
  celltype = annotationnames[1 : ncol(heatmap_df)],
  col = list(celltype = custom_colors)
  )

#annotate genes of interest
annot_genes <- HeatmapAnnotation(
  GeneOfInterest = anno_mark(at = match(genes_to_anno, rownames(heatmap_df)),
                             labels = genes_to_anno),
  which = "row"
  ) #show_annotation_name = T
 
#K means with 7 groups
heatmap_trajectory_hc <- draw(Heatmap(heatmap_df, top_annotation = annot_celltype, right_annotation = annot_genes,
                                      name = "z-score", col = colorRamp2(c(min(heatmap_df), 0, max(heatmap_df)), c("blue", "lightyellow", "red")), #coloring heatmap
                                      km = 7, row_km_repeats = 1, row_dend_reorder = F, cluster_rows = T, clustering_method_rows = "single", cluster_row_slices = F, #clustering
                                      row_title_rot = 0, show_row_names = F, 
                                      cluster_columns  = F))

#later: put cluster_rows to F
```

```{r extract genes}
orders <- row_order(heatmap_trajectory_hc)
genelist <- rownames(dfhc)
genelistdf <- as.data.frame(genelist)
genelistdf["cluster"] = ""
for (x in as.character(1:7)) {
  tmplist = orders[x]
  genelist2 = genelist[tmplist[[1]]]
  genelistdf[genelist2,]=paste("cluster",x)
}
genelistdf = genelistdf[genelistdf$cluster!="",]
genelistdf$cluster=NULL
genelistdf$gene = rownames(genelistdf)
genelistdf_hc = genelistdf
```
...
custom_colors_b3sub <- c("basal III KRT15+" = "#6573A0", "basal III ASS1+" = "#B0B9E6", "basal II" = "#66C2A5", "basal I" = "#FFD92F", "basal IV" = "#A6D854", "stratum spinosum" = "#FC8D62", "stratum granulosum" = "#E78AC3")
DimPlot(roj_rds[, roj_rds$disease == "HC"], cols = custom_colors_b3sub, repel = T,  label = T,  label.box = T) + NoLegend()
```
...
VlnPlot(roj_rds[, roj_rds$disease == "HC"], cols = custom_colors, features = "TFAP2A")

FeaturePlot(roj_rds[, roj_rds$disease == "AD"], features = c('TFAP2A'), ncol = 1)
VlnPlot(roj_rds[, roj_rds$disease == "AD"], cols = custom_colors, features = "TFAP2A")
...
VlnPlot(roj_rds, features = c('TSLP'), split.by = "disease", cols = custom_colors) + theme(axis.title.x=element_blank())
