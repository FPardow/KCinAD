...
VlnPlot(roj_rds, group.by = "svm_annotation", split.by = "disease", features = c("KRT5", "KRT14", "KRT1", "KRT10", "DSC1", "DSG1", "COL17A1", "ITGB1", "ITGA6", 'KRT15', "IVL", 'FLG', "LOR", 'KRT6A', 'KRT16', "S100A8", "S100A9", "CA2"), stack = T, flip = T, cols = colors_disease) + theme(axis.title.x = element_blank())
...
```{r new heatmap without cell cycle}
...
#heatmaps  for early / late trajectory genes
setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
heatmap_trajectory_hcad_early_k5 <- readRDS("heatmap_trajectory_hcad_early_k5.rds")

# heatmap_trajectory_hcad_early_k5 <- draw(
...
# 
# setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
# saveRDS(object = heatmap_trajectory_hcad_early_k5, file = "heatmap_trajectory_hcad_early_k5_6.rds")

heatmap_trajectory_hcad_late <- draw(Heatmap(heatmap_hcad_late, top_annotation = annot_celltype_hcad,
                                      col = colorRamp2(c(min(heatmap_ad), 0, max(heatmap_ad)), c("blue", "lightyellow", "red")),
                                      name = "z-score", km = 3, row_km_repeats = 3, 
                                      row_dend_reorder = F, cluster_rows = T, clustering_method_rows = "single", cluster_row_slices = F, 
                                      row_title_rot = 0, show_row_names = F, show_column_names = F,
                                      cluster_columns  = F)) 

genes_to_anno_hcearly <- c("KRT15", "ASS1")
...
heatmap_trajectory_hcad_early_k5 + annot_genes_hcearly
```
...
#annotate cell types -> hiding legend here
annot_celltype_hcad <- HeatmapAnnotation(
  celltype = c(svm_annotation_hc, svm_annotation_ad),
  col = list(celltype = colors_subtypes),
  show_legend = FALSE
...
for (method in clust_dist){
  ...
  heatmap_all_compare[[method]] <- ggplotify::as.ggplot(
         Heatmap(heatmap_hcad_early, right_annotation = annot_genes, top_annotation = annot_celltype_hcad, 
                                      col = colorRamp2(c(min(heatmap_ad), 0, max(heatmap_ad)), c("blue", "lightyellow", "red")),
...
summary(row_order(heatmap_trajectory_hcad_early_k5))

c1 <- heatmap_hcad_early[row_order(heatmap_trajectory_hcad_early_k5)[[1]], ]
c2 <- heatmap_hcad_early[row_order(heatmap_trajectory_hcad_early_k5)[[2]], ]
c3 <- heatmap_hcad_early[row_order(heatmap_trajectory_hcad_early_k5)[[3]], ]
c4 <- heatmap_hcad_early[row_order(heatmap_trajectory_hcad_early_k5)[[4]], ]
c5 <- heatmap_hcad_early[row_order(heatmap_trajectory_hcad_early_k5)[[5]], ] 

heatmap_hcad_early_ASS1down <- c(rownames(c2))
...
list <- list("Early_ASS1down" = ASS1down_GO,
             ...
             "Early_c1" = rownames(c1)
             )
write.xlsx(list, file = "GO Heatmap HCADtrajectory early.xlsx")
```

#create nice unannotated early heatmap and check if I can annotate it later 
#late heatmap
