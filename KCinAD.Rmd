...
# Functional analysis basal cell types
...
## Visualize only highly variable / differentially expressed genes early in the pseudotime trajectory 
...
barplot(ASS1down_GO, showCategory = ASS1down_GO$Description[c(1, 3, 17)])
...
barplot(KRT15down_GO, showCategory = KRT15down_GO$Description[c(4, 12)])
...
table("KRT16" %in% c(heatmap_hcad_early_ASS1down, heatmap_hcad_early_nochange))
table("LY6D" %in% c(heatmap_hcad_early_KRT15down, heatmap_hcad_early_nochange))
...
], features = c("CCL2", "POSTN", "IL1R2", "DCN","ID1", "IRF1", "SOCS3")) & theme(legend.position = "right")
> FeaturePlot(roj_rds[, roj_rds$disease == "AD"], features = c("IGFBP3", "KRT6A", "KRT16", "S100A7", "S100A8", "KRT17", "CASP14", "SERPINB2", "DSG1")) & theme(legend.position = "right")

table(markers[markers$AD__avg_log2FC < 0 & !is.na(markers$AD__avg_log2FC), "gene"] %in% c(heatmap_hcad_early_KRT15down, heatmap_hcad_early_nochange))

markers[markers[markers$AD__avg_log2FC < 0 & !is.na(markers$AD__avg_log2FC), "gene"] %in% c(heatmap_hcad_early_KRT15down), "gene"]
table(markers[markers$AD__avg_log2FC < 0 & !is.na(markers$AD__avg_log2FC), "gene"] %in% c(heatmap_hcad_early_KRT15down))

"LGALS1" %in% markers[markers$AD__avg_log2FC < 0 & !is.na(markers$AD__avg_log2FC), "gene"]
"LGALS1" %in% c(heatmap_hcad_early_KRT15down)
```

## Visualize only highly variable / differentially expressed genes late in the pseudotime trajectory 
