...
## Using tradeseq to look at gene expression during pseudotime

Ward says expressed in at least 1 % of cells - not sure how he comes up with that 

```{r select highly expressed genes}
counts <- as.data.frame(roj_rds@assays$RNA@counts)
roj_rds@assays$RNA@data
...
gene_expr$selected <- "no"
gene_expr[gene_expr$log10_counts > 3.1 & gene_expr$percent_cell > 0.2, ]$selected <- "yes"
color <- c("black","red")
names(color) <- c("no","yes")

#plot
library(ggrepel)
ggplot(data = gene_expr, aes(x = log10_counts, y = percent_cell, col = selected)) + geom_point() + 
...
```{r figure out best number of k's to fit the NB-GAM}
set.seed(123)
icMat <- evaluateK(counts = roj_rds_sce@assays@data$counts, sds = SlingshotDataSet(roj_rds_sce), parallel = T) #k = 3:10, nGenes = 500, verbose = FALSE, plot = TRUE
print(icMat[1:2, ])
```
...
```{r}
set.seed(123)
sce <- fitGAM(counts = counts, sds = SlingshotDataSet(roj_rds_sce), nknots = 6)

roj_rds_sce@assays@data$counts
roj_rds@assays$RNA@data
roj_rds@assays$RNA@counts # = counts
```


```{r calculate NB-GAM}
highexpr <- rownames(gene_expr[gene_expr$color == "yes", ])
...
BPPARAM$workers <- 6 #use 6 cores, thats also the default
...
roj_rds_slb_gam <- fitGAM(counts = roj_rds@assays$RNA@data, sds = SlingshotDataSet(roj_rds_sce),
                          genes = intersect(roj_rds@assays$RNA@var.features, rownames(gene_expr[gene_expr$color == "yes", ])), #which genes to fit, default is all, 
                          nknots = 6, #nknots = 6 is default but can be changed) 
                          parallel = T)
# BPPARAM = BiocParallel::bpparam(bpparamClass = MulticoreParam)

setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
roj_rds_slb_gam <- readRDS("roj_rds_slb_gam.rds")

assoRes_all <- associationTest(roj_rds_slb_gam, lineages = T)

# setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
# saveRDS(roj_rds_slb_gam, "roj_rds_slb_gam.rds")
# write.xlsx(assoRes_all, file = "assoRes_all.xlsx")

pseudotime_genes <- assoRes_all[assoRes_all$pvalue_1 < 0.05, ]
pseudotime_genes <- intersect(rownames(pseudotime_genes), rownames(gene_expr[gene_expr$color == "yes", ]))
```

understand code, 
talk to ward about integrated vs non-integrated -> optimize heatmap clustering 
...
#healthy trajectory
genes_heatmap <- c(pseudotime_genes, gene_anno)
