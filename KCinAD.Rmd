...
<!-- Here we choose a subset of highly expressed genes that we could later use or overlap with highly variable expression -->
...
pseudotime_genes <- diffex_trajectory[diffex_trajectory$pvalue_1 < 0.05 & !is.na(diffex_trajectory$pvalue_1), ]
...
#later: put tree height cluster_rows to F / 0
#work on adjusting clustering
#extract genes 
...
annot_celltype_hc <- HeatmapAnnotation(celltype = svm_annotation_hc, col = list(celltype = custom_colors))
...
for (method in clust_dist){
  heatmap_all_compare[[method]] <- as.ggplot(
    Heatmap(heatmap_hc, right_annotation = annot_genes, top_annotation = annot_celltype_hc,
            col = colorRamp2(c(min(heatmap_hc), 0, max(heatmap_hc)), c("blue", "lightyellow", "red")),
            name = "z-score", clustering_distance_rows = method, 
            row_dend_reorder = F, cluster_rows = T,
            clustering_method_rows = "single", cluster_row_slices = F,
            row_title_rot = 0, show_row_names = F, show_column_names = F, cluster_columns  = F)
                                             )
}
...
setwd("C:/Users/spare/OneDrive - Radboudumc/Results/KCsinAD/FPcomp/KCinAD")
pdf("Heatmap_HC_clusteringDistance.pdf", height = 8, width = 12)
plot_grid(plotlist = heatmap_all_compare) #another way of multiplotting
dev.off()
```


```{r extract genes}
summary(row_order(heatmap_trajectory_hc))


orders <- row_order(heatmap_trajectory_hc)
genelist <- rownames(heatmap_df)
...
h1 <- row.names(heatmap_hc)[row_order(heatmap_trajectory_hc)[[1]]] #k means: $`1`, split: [[1]]
h2 <- row.names(z_limma)[row_order(pearson14)[[2]]]
h3 <- row.names(z_limma)[row_order(pearson14)[[3]]]
h4 <- row.names(z_limma)[row_order(pearson14)[[4]]]
h5 <- row.names(z_limma)[row_order(pearson14)[[5]]]
h6 <- row.names(z_limma)[row_order(pearson14)[[6]]]
h7 <- row.names(z_limma)[row_order(pearson14)[[7]]]
h8 <- row.names(z_limma)[row_order(pearson14)[[8]]]
h9 <- row.names(z_limma)[row_order(pearson14)[[9]]]
h10 <- row.names(z_limma)[row_order(pearson14)[[10]]]
h11 <- row.names(z_limma)[row_order(pearson14)[[11]]]
h12 <- row.names(z_limma)[row_order(pearson14)[[12]]]
h13 <- row.names(z_limma)[row_order(pearson14)[[13]]]
h14 <- row.names(z_limma)[row_order(pearson14)[[14]]]

c1 <- c(h7, h8)
c2 <- c(h9, h10, h11, h12, h13, h14)
c21 <- c(h9, h10)
c22 <- c(h12, h13, h14)
c23 <- c(h11)
c3 <- c(h1, h2, h3, h4, h5, h6)
c31 <- c(h4, h5, h6)
c32 <- c(h2, h3)
c33 <- c(h1)
```
...
annot_celltype <- HeatmapAnnotation(celltype = svm_annotation_ad, col = list(celltype = custom_colors))
...
#K means with 7 groups
heatmap_trajectory_ad <- draw(Heatmap(heatmap_ad, right_annotation = annot_genes, top_annotation = annot_celltype,
...
#K means with 7 groups
heatmap_trajectory_ad <- draw(Heatmap(heatmap_hcad, right_annotation = annot_genes, top_annotation = annot_celltype, 
                                      col = colorRamp2(c(min(heatmap_ad), 0, max(heatmap_ad)), c("blue", "lightyellow", "red")),
                                      name = "z-score", 
                                      km = 7, row_km_repeats = 1, row_dend_reorder = F, cluster_rows = T, clustering_method_rows = "single", cluster_row_slices = F, #clustering
                                      row_title_rot = 0, show_row_names = F, show_column_names = F,
