...
VlnPlot(wang_filtered, features = c("KRT1", #differentiated KCs
                                    "KRT5", #basal KCs
                                    "KRT15", #stem cells
                                    "MITF", "MLANA", "CDH3", #melanocytes
                                    "SOX9", "RUNX1", #hair follicle
                                    "PECAM1", "CLDN5", #endothelial cells
                                    "CD207", "CD86" #langerhans cells
                                    ), flip = T, stack = T,
        cols = c(rep("#53B400", 3), rep("#F8766D", 3), rep("#E76BF3", 2), rep("#00B6EB", 2), rep("#A3A500", 2))) +
...
cluster_names <- vector()
for (i in celltypes){
  cluster_names <- append(cluster_names, paste("reconstructed", i, sep = " "))
}
...
confusion_matrix <- tibble::rownames_to_column(confusion_matrix, var = "differentially expressed markers")

#create gt table with percentage formatting and color mapping
confusion_matrix_gt <- gt(confusion_matrix) %>%
  fmt_number(columns = -"differentially expressed markers", decimals = 1) %>%  # Format numbers with 1 decimals
  fmt(columns = -"differentially expressed markers", fns = function(x) paste0(x, "%")) %>%  # Append '%' sign
  data_color(columns = -"differentially expressed markers", palette = c("white", "#08306b"))
...
```{r lineplot KC markers}
# wang_kc <- readRDS(paste0(save.dir, "wang_kc.rds"))

DefaultAssay(wang_kc) <- "RNA"
Idents(wang_kc) <- wang_kc$annotation

#extract the gene counts per pseudobulk cluster
counts_basalIII <- wang_kc[ , wang_kc$annotation == "basal III"]@assays[["RNA"]]$data
counts_basalII <- wang_kc[ , wang_kc$annotation == "basal II"]@assays[["RNA"]]$data
counts_basalI <- wang_kc[ , wang_kc$annotation == "basal I"]@assays[["RNA"]]$data
counts_basalIV <- wang_kc[ , wang_kc$annotation == "basal IV"]@assays[["RNA"]]$data
counts_spin <- wang_kc[ , wang_kc$annotation == "stratum spinosum"]@assays[["RNA"]]$data
counts_gran <- wang_kc[ , wang_kc$annotation == "stratum granulosum"]@assays[["RNA"]]$data

#filter the counts for the required marker and prepare the data frame
counts_KCmarkers <- data.frame(
  counts = median(counts_basalIII[c("KRT5"), ]),
  gene = "KRT5",
  cell_type = "basal III"
  ) 

counts_KCmarkers <- counts_KCmarkers %>%
  add_row(counts = median(counts_basalII[c("KRT5"), ]), gene = "KRT5", cell_type = "basal II") %>% 
  add_row(counts = median(counts_basalI[c("KRT5"), ]), gene = "KRT5", cell_type = "basal I") %>% 
  add_row(counts = median(counts_basalIV[c("KRT5"), ]), gene = "KRT5", cell_type = "basal IV") %>%
  add_row(counts = median(counts_spin[c("KRT5"), ]), gene = "KRT5", cell_type = "stratum spinosum") %>% 
  add_row(counts = median(counts_gran[c("KRT5"), ]), gene = "KRT5", cell_type = "stratum granulosum") 

counts_KCmarkers <- counts_KCmarkers %>%
  add_row(counts = median(counts_basalIII[c("KRT14"), ]), gene = "KRT14", cell_type = "basal III") %>% 
  add_row(counts = median(counts_basalII[c("KRT14"), ]), gene = "KRT14", cell_type = "basal II") %>% 
  add_row(counts = median(counts_basalI[c("KRT14"), ]), gene = "KRT14", cell_type = "basal I") %>% 
  add_row(counts = median(counts_basalIV[c("KRT14"), ]), gene = "KRT14", cell_type = "basal IV") %>%
  add_row(counts = median(counts_spin[c("KRT14"), ]), gene = "KRT14", cell_type = "stratum spinosum") %>% 
  add_row(counts = median(counts_gran[c("KRT14"), ]), gene = "KRT14", cell_type = "stratum granulosum") 

counts_KCmarkers <- counts_KCmarkers %>%
  add_row(counts = median(counts_basalIII[c("KRT1"), ]), gene = "KRT1", cell_type = "basal III") %>% 
  add_row(counts = median(counts_basalII[c("KRT1"), ]), gene = "KRT1", cell_type = "basal II") %>% 
  add_row(counts = median(counts_basalI[c("KRT1"), ]), gene = "KRT1", cell_type = "basal I") %>% 
  add_row(counts = median(counts_basalIV[c("KRT1"), ]), gene = "KRT1", cell_type = "basal IV") %>%
  add_row(counts = median(counts_spin[c("KRT1"), ]), gene = "KRT1", cell_type = "stratum spinosum") %>% 
  add_row(counts = median(counts_gran[c("KRT1"), ]), gene = "KRT1", cell_type = "stratum granulosum") 

counts_KCmarkers <- counts_KCmarkers %>%
  add_row(counts = median(counts_basalIII[c("KRT10"), ]), gene = "KRT10", cell_type = "basal III") %>% 
  add_row(counts = median(counts_basalII[c("KRT10"), ]), gene = "KRT10", cell_type = "basal II") %>% 
  add_row(counts = median(counts_basalI[c("KRT10"), ]), gene = "KRT10", cell_type = "basal I") %>% 
  add_row(counts = median(counts_basalIV[c("KRT10"), ]), gene = "KRT10", cell_type = "basal IV") %>%
  add_row(counts = median(counts_spin[c("KRT10"), ]), gene = "KRT10", cell_type = "stratum spinosum") %>% 
  add_row(counts = median(counts_gran[c("KRT10"), ]), gene = "KRT10", cell_type = "stratum granulosum") 
...
DimPlot(rojahn_kc, split.by = "state", group.by = "svm_annotation") + labs(color = "cell type") + scale_colour_discrete(type = colors_subtypes, labels = scales::label_wrap(10)) + ggtitle(element_blank()) #supplemental figure 4a
...
```{r barplot CC percentages - cell states together}
DefaultAssay(rojahn_kc) <- "RNA"

rojahn_kc$state_svm <- paste0(rojahn_kc$state, "_", rojahn_kc$svm_annotation)
rojahn_kc$state_svm <- paste0(rojahn_kc$state, "_", rojahn_kc$svm_annotation)

# calculate percentages
phase_percentage_all <- percentage_df(rojahn_kc, cell_annotation = "Phase", variable = "state_svm" ,facet = FALSE)

# factor ordering for consistent x-axis
...
phase_percentage_all$Variable <- factor(phase_percentage_all$Variable, levels = cell_order)
phase_percentage_all$Celltype <- factor(phase_percentage_all$Celltype, levels = c("G1", "S", "G2M"))

# plot, figure 3c
ggplot(phase_percentage_all, aes(fill = Celltype, y = percentage, x = Variable)) + 
  geom_bar(position = "stack", stat = "identity") +
  geom_text(aes(label = ann), position = position_stack(vjust = 0.5), size = 2.5) + 
...
```{r barplot CC percentages - cell states separate}
DefaultAssay(rojahn_kc) <- "RNA"
Idents(rojahn_kc) <- "Phase"
